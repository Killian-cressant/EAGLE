...
for (( i=1; i<=NUM_TRIALS; i++ )); do
    ...
    echo "Running: lr=$lr, ws=$ws, tw=$tw, top_k=$k, output_dim=$o, hidden_dim=$hd, num_layers=$nl, pos_weight_exp=$pwe"
    ...
    if [[ -n "$acc" ]]; then
        if (( $(echo "$acc > $best_acc" | bc -l) )); then
            best_acc=$acc
            best_args="lr=$lr ws=$ws tw=$tw top_k=$k output_dim=$o hidden_dim=$hd num_layers=$nl pos_weight_exp=$pwe"
